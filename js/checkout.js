        ...
        <tr>
            ...
            <td data-label="Qtd">
                <input 
                    ...
                    value="${item.qtd}" 
                    min="1" 
                    onchange="handleAlterarQuantidade(${item.id}, this.value, '${item.tipo}')"
                >
            </td>
            <td data-label="Preço Unit.">R$ ${item.precoUnit.toFixed(2).replace('.', ',')}</td>
            <td data-label="Subtotal">R$ ${(item.precoUnit * item.qtd).toFixed(2).replace('.', ',')}</td>
            <td data-label="Ações">
                <button class="btn-remover" onclick="handleRemoverItem(${item.id}, '${item.tipo}')">Remover</button>
            </td>
        </tr>
    `).join('');
...
async function handleRemoverItem(id, tipo = 'produto') {
    const container = document.getElementById('mensagem-remover');
    carrinho.remover(id, tipo);
    await renderizarResumoCarrinho();
    container.innerText = tipo === 'oferta' ? 'Oferta removida do carrinho!' : 'Produto removido do carrinho!';
    container.classList.add('show');
    setTimeout(() => container.classList.remove('show'), 2000);
}

async function handleAlterarQuantidade(id, qtd, tipo = 'produto') {
    carrinho.alterarQuantidade(id, qtd, tipo);
    await renderizarResumoCarrinho();
}
